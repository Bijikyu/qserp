...
const rawMaxSize = parseInt(process.env.QSERP_MAX_CACHE_SIZE) || 1000;
const MAX_CACHE_SIZE = Math.max(10, Math.min(rawMaxSize, 50000)); // Constrain between 10-50000 entries
...
function handleAxiosError(error, contextMsg) {
        ...
        try {
                ...
                // Context includes the operation being performed when error occurred
...
/**
 * Performs intelligent cache cleanup to prevent memory exhaustion
 * 
 * PERFORMANCE STRATEGY: This function implements a two-tier cleanup approach:
 * 1. First removes expired entries (based on TTL)
 * 2. If still over threshold, removes oldest entries (LRU-style)
 * 
 * This approach preserves recently accessed data while ensuring memory bounds.
 * The cleanup is triggered proactively to avoid performance degradation.
 */
function performCacheCleanup() {
        if (DEBUG) { logStart('performCacheCleanup', `${cache.size} entries`); }
        
        const now = Date.now();
        let removedCount = 0;
        
        // Phase 1: Remove expired entries first (prioritize expired over old)
        // CLEANUP STRATEGY: Always remove expired entries regardless of cache size,
        // as they provide no value and consume memory unnecessarily
        ...
        // Phase 2: If still over threshold, remove oldest entries (LRU-style)
        // SIZE ENFORCEMENT: After removing expired entries, enforce hard size limit
        // by removing oldest remaining entries to prevent unbounded memory growth
        if (cache.size > MAX_CACHE_SIZE) {
                // Convert Map to array and sort by timestamp (oldest first)
                // SORT RATIONALE: Timestamp-based sorting implements LRU behavior where
                // oldest entries are removed first, preserving recently accessed data
                const entries = Array.from(cache.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp);
                
                // Calculate exact number of entries to remove
                // PRECISION: Remove only what's necessary to reach MAX_CACHE_SIZE,
                // avoiding over-cleanup that would waste previously cached results
                const toRemove = cache.size - MAX_CACHE_SIZE;
                
                // Remove oldest entries up to the calculated limit
                // SAFETY BOUNDS: Double-check array bounds to prevent index errors
                // even if cache size calculations are somehow incorrect
        ...
        if (DEBUG) { logReturn('performCacheCleanup', `Removed ${removedCount} entries, ${cache.size} remaining`); }
        return removedCount;
}
...
module.exports = {
       ...
       , performCacheCleanup    // Intelligent cache cleanup for memory management
