...
async function fetchSearchItems(query, num) { //accept optional num for result count
        ...
        try {
               ...
               // Normalize cache key for better hit ratio while preserving search intent
               // SECURITY CONSIDERATION: Normalization improves efficiency without compromising search accuracy
               const normalizedQuery = query.trim().toLowerCase();
               const cacheKey = num ? `${normalizedQuery}:${num}` : normalizedQuery; //use num in key so cached result counts stay separate
...
function performCacheCleanup() {
        ...
        // Phase 1: Remove expired entries first
        ...
        // Phase 2: If still over threshold, remove oldest entries
